...
def accuracy(params, non_trainable_params, batch):
    """Calculates accuracy (or number of correct guesses) for a given batch"""
    inputs, targets = batch
    target_class = jnp.argmax(targets, axis=1)
    predicted_class = jnp.argmax(net_predict(params, non_trainable_params, inputs)[0], axis=1)
...
def main():
    ...
    @jit
    def update(i, opt_state, non_trainable_params, batch):
        def loss(params, non_trainable_params, batch):
            """Calculates the loss of the network as a single value / float"""
            inputs, targets = batch
            predictions, non_trainable_params = net_predict(params, non_trainable_params, inputs)
            return categorical_cross_entropy(predictions, targets), non_trainable_params

        params = get_params(opt_state)
        grads, non_trainable_params = grad(loss, has_aux=True)(params, non_trainable_params, batch)
        return opt_update(i, grads, opt_state), non_trainable_params

    _, init_params, non_trainable_params = net_init(rng, (-1, 28 * 28))
    ...
    for epoch in (t := trange(num_epochs)):
        for batch in range(num_batches):
            opt_state, non_trainable_params = update(next(itercount), opt_state, non_trainable_params, next(batches))

        params = get_params(opt_state)
        train_acc = accuracy(params, non_trainable_params, (train_images, train_labels))
        test_acc = accuracy(params, non_trainable_params, (test_images, test_labels))
        t.set_description_str("Accuracy Train = {:.2%}, Accuracy Test = {:.2%}".format(train_acc, test_acc))
    print("Training Complete.")

    # Visual Debug After Training
    visual_debug(get_params(opt_state), non_trainable_params, test_images, test_labels)

def visual_debug(params, non_trainable_params, test_images, test_labels, starting_index=0, rows=5, columns=10):
        ...
        def render_images(self):
            ...
            for j in range(rows):
                for k in range(columns):
                    output = net_predict(params, non_trainable_params, test_images[i].reshape(1, test_images[i].shape[0]))[0]
